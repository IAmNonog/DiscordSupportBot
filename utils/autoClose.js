...
async function autoCloseMain(client) {
    try {
        ...
        for (const thread of threads.threads.values()) {
            console.log("[AUTO-SCAN] - Scan Ticket : " + thread.name);
            ...
            if (lastMessage.createdTimestamp < XHoursAgo && thread.name.includes('[' + TICKET_PREFIX)) {
                console.log("[AUTO-SCAN] - Old Message");
                ...
                if (lastMessage.content == TICKET_CLOSE_ASK || isSupportMessage || lastMessage.content == AUTO_RELAUNCH_USER) {
                    console.log("[AUTO-SCAN] - Need close");
